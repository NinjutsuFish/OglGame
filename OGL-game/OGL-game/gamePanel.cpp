...
void gamePanel::init() {
	...
	else//is initialised !!!! YAYYY:)
	{
		...
		else
		{
			...
			myRenderer = new MasterRender();
		
			
			myEntity[0] = Entity(texmodel, glm::vec3(-8, 0, -25), 0, 0, 0, 1);
			myEntity[1] = Entity(texmodel, glm::vec3(8, 0, -25), 0, 0, 0, 1);
...
void gamePanel::updateGame() {
		...
		myEntity[0].IncreaseRotation(0, 5, 0);
		myEntity[1].IncreaseRotation(0, -5, 0);
...
void gamePanel::renderGame() {
	//DRAW MODELSSSSSSS
		...
		myRenderer->processEntity(myEntity[0]);
		myRenderer->processEntity(myEntity[1]);
		myRenderer->Render(mylight, camera);

		//render.render(myEntity[0], myshader, camera);
		//render.render(myEntity[1], myshader, camera);
		//myshader->stop();
